...
export interface LeadAuditEmailPayload {
  currentMonthlySpend: number;
  potentialMonthlySavings: number;
  estimatedAnnualSavings: number;
  /** Short lines for the email (already human-readable). */
  bullets: string[];
}

export interface SendLeadConfirmationParams {
  leadId: string;
  to: string;
  companyName?: string | null;
  /** Public report URL, e.g. https://app.example.com/audit/SHAREID */
  reportUrl?: string | null;
  /** Site origin for generic CTA when there is no audit payload. */
  siteHomeUrl: string;
  /** When null, a shorter follow-up without figures is sent. */
  audit: LeadAuditEmailPayload | null;
  /** Same bytes as the in-app PDF download (optional). */
  pdfAttachment?: { filename: string; content: Buffer };
}

function greetingLine(companyName?: string | null): string {
...
function greetingHtml(companyName?: string | null): string {
  const c = companyName?.trim();
  if (c) {
...
function buildBodies(params: SendLeadConfirmationParams): {
  html: string;
  text: string;
} {
...
<body style="font-family:system-ui,Segoe UI,sans-serif;line-height:1.55;color:#0f172a;max-width:42rem">
  ...
  const bullets =
    a.bullets.length > 0 ? a.bullets : DEFAULT_RECOMMENDATION_BULLETS;
...
export async function sendLeadConfirmationEmail(
  params: SendLeadConfirmationParams
